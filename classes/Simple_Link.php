 ...
 *
 */
class Simple_Link extends SL_post_type_tax{
	...
	public function register_sl_post_type() {
		...
		$this->register_post_type( self::POST_TYPE, apply_filters( 'simple-links-register-post-type', $args ) );
	...
	function meta_save(){
        ...
        if( empty( $post->post_type ) || $post->post_type != self::POST_TYPE ) return;
	...
	function meta_box($post){
		...
		foreach( $this->simple_link_meta_fields as $box){
			if( ($box != 'additional_fields') && ($box != 'target') ){
				add_meta_box( $box.'_links_meta_box', ucwords( str_replace('_', ' ', $box )) , array( $this, 'link_meta_box_output' ), $post->type , 'advanced' , 'high', $box );
			}
		}
